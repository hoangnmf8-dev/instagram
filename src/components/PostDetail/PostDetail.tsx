...
export default function PostDetail({openPostDetail, setOpenPostDetail, postId}: Props) {
  ...
  const {data: postDetail} = useQuery({
    queryKey: getPostsNewfeedDetailKey(postId),
  ...
  const {data: commentData, isLoading: isLoadingComment} = useQuery({
    queryKey: getPostCommentKey(postId),
  ...
  const mutationCreateRepliesComment = useMutation({
    ...
    onSuccess: () => {
      ...
      query.invalidateQueries({queryKey: getPostCommentKey(postId)});
      query.invalidateQueries({queryKey: getRepliesCommentKey(postId, commentId)});
    }
  })
  ...
  const mutationLikePost = useMutation({
    ...
    onSuccess: () => {
      query.invalidateQueries({queryKey: getPostsNewfeedKey});
      query.invalidateQueries({queryKey: getPostsNewfeedDetailKey(postId)});
    }
  })
          ...
          <DialogHeader className='sr-only'>
            ...
            <div className='flex'>
              ...
              <div className='basis-3/5'>
                <UserInfo userData={userData?.data} onClick={handleClickUser} className='p-4'/>
                <hr className='my-2'/>
                ...
                <div className='overflow-auto h-90 [&::-webkit-scrollbar]:hidden p-4'>
                  {isLoadingComment ? 
                    ...
                    :commentData?.data?.comments?.map(comment => <PostDetailComment key={comment?._id} comment={comment} postId={postId} setIsReply={setIsReply} setValue={setValue} setFocus={setFocus} setCommentId={setCommentId}/>)}
                </div>
